...
export default function TestPage() {
  ...
  async function uploadTestFile() {
    ...
    try {
      const filePath = `${userId}/test-${Date.now()}.${selectedFile.type === 'image/png' ? 'png' : 'svg'}`
      ...
      console.log('Existing files:', existingFile)
      ...
      const { data: imageRecord, error: insertError } = await supabase
        ...
        .insert({
          ...
          prompt: 'Test upload',
          style: 'test',
          format: selectedFile.type === 'image/png' ? 'PNG' : 'SVG',
          keywords: ['test']
        })
        .select()
        .single()
  ...
  async function runAllTests() {
    ...
    try {
      ...
      try {
        ...
        if (!user) {
          console.error('❌ No user authenticated')
          results['Upload Error'] = 'No user authenticated'
          return
        }
        ...
        if (uploadError) {
          ...
          results['Upload Error'] = JSON.stringify({
            message: uploadError.message,
            name: uploadError.name
          }, null, 2)
        } else {
